...
keep:
  ...
  - tools/set-git-config.ps1 stays idempotent and reports only changed Git config or Editor checkout preference values plus completion, not already-correct keys or routine successful checks.
