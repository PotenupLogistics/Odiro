...
- If Editor Reload MCP reports `crash_report_pending` or `blockedByCrashReporter=true`, stop editor launch/restart attempts and close or ask the user to close the Unreal Crash Report window before retrying recover. A `CrashReportClientEditor` process with `-MONITOR=<live editor pid>` is a normal active monitor and should not be treated as a crash dialog.
