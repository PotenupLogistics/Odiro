...
## Entry Points
- `SimulatorLaunchSubsystem.h` and `.cpp`: simulator subprocess launcher, `RunPreview.bat` development fallback, status polling, fixed-step FPS save helper, run queue JSON read/write/validation (schema `episode_run_queue`, setup schema `scenario_actor_spawn_mvp`)
- `ScenarioEditorLaunchSubsystem.h` and `.cpp`: MainMenu-to-`ScenarioEditorMap` navigation, pending scenario setup path handoff, editor controller auto-load attempt
...
- `Widget/MainMenuWidget.h` and `.cpp`: document-aligned MainMenu sidebar sections for scenario, policy, experiment config, run status, and experiment result; includes setup/run queue selector, simulator launcher, new/load scenario editor entry, policy JSON external editor launch, status/report/log preview UI
- `Private/Platform/Tests`: launcher command contract, terminal-state automation, simulator process helper automation

## Notes
- Platform UI does not load `ScenarioSimulationMap` directly; simulator work runs in a separate process
...
- MainMenu opens `ScenarioEditorMap` through `UScenarioEditorLaunchSubsystem`; selected scenario setup auto-load requires the editor map's GameMode to use `AScenarioEditorController`
- AI analysis uses report JSON to resolve scenario setup/DeliveryBotSetup paths and sends absolute local file paths to the AI server; the UI does not send API keys
