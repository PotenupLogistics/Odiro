...
class AnalysisTextBuilder:
    ...
    def _environment_evidence_text(
        ...
        *,
        ...
        parts: list[str] = []
        static_collision_count = self._evidence_metric_total(evidence, "static_obstacle_collision_count")
        collision_count = static_collision_count or metric_values.get("collision_count", 0)
        if collision_count:
            collision_episode_count = self._evidence_episode_count(evidence, "static_obstacle_collision_count")
            display_episode_count = collision_episode_count or episodes_count
            episode_text = f"분석 가능한 {display_episode_count}개 에피소드에서 " if display_episode_count else ""
            parts.append(f"{episode_text}정적 장애물 충돌이 총 {collision_count}회 확인되었습니다.")
