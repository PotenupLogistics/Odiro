...
알파 단계에서는 보행자 요청이 있어도 background.count는 0, encounters는 []로 고친다.
...
robot start/goal anchor만 요청된 prompt인데 pedestrians.encounters가 있으면 []로 제거하고 background.count는 0으로 둔다.
