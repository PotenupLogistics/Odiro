> Temporary AI-side catalog
>
> 이 문서는 Unreal 쪽에서 공유받은 scenario environment catalog를
> v2 Agent LLM context에서 참고하기 위해 Agents 문서에 임시로 둔 것이다.
>
> surface id, prop id, persona id, encounter type 등은 AI와 UE가 함께 맞춰야 하는 공유 계약 성격이 있으므로,
> 추후 canonical 문서가 `contracts/specs/` 하위에 추가되면 이 파일은 제거하고 해당 contract 문서를 참조한다.
