...
현재 생성된 policy knowledge card 11개가 MVP 정책의 어떤 상황, 액션, 파라미터를 커버하는지 정리한다.

## 2. 현재 카드 요약

- 총 card 수: 11
- runtime chunk 수: 17
- source별 card 수: KOR-003 9개, KOR-004 2개
- category별 card 수: crosswalk_operation 1, emergency_stop 2, speed_policy 3, terrain_or_dynamic_safety 2, perception_requirement 1, sidewalk_operation 1, operator_control 1

## 3. MVP 상황 커버리지

| MVP 상황 | 연결 cardId | category | Decision Request 필드 | selectedAction | 부족한 근거 여부 |
| --- | --- | --- | --- | --- | --- |
| PedestrianAhead | CARD-KOR-003-perception_requirement-001; CARD-KOR-003-sidewalk_operation-001; CARD-KOR-004-crosswalk_operation-001 | perception_requirement; sidewalk_operation; crosswalk_operation | detectedObjects[]; detectedObjects[].isOnPath; event.confidence; environments[].state | SlowDown; Stop; LocalAvoidance; YieldWait | ready |
...
| CrosswalkApproach | CARD-KOR-003-sidewalk_operation-001; CARD-KOR-004-crosswalk_operation-001 | sidewalk_operation; crosswalk_operation | environments[].type; environments[].state; detectedObjects[].isOnPath | Stop; YieldWait; Continue | ready |
...
| --- | --- | --- | --- | --- |
| Continue | CARD-KOR-003-sidewalk_operation-001; CARD-KOR-004-crosswalk_operation-001 | sidewalk_operation; crosswalk_operation | lowSpeedZoneSpeedKmh; waitTimeoutSec | partial: 횡단보도 상태 기반 계속 주행 판단은 후속 request field 설계 필요 |
| SlowDown | CARD-KOR-003-speed_policy-001; CARD-KOR-003-speed_policy-002; CARD-KOR-003-perception_requirement-001; CARD-KOR-004-speed_policy-001; CARD-KOR-004-crosswalk_operation-001 | speed_policy; perception_requirement; crosswalk_operation | maxSpeedKmh; lowSpeedZoneSpeedKmh; safeDistanceCm | ready |
| Stop | CARD-KOR-003-emergency_stop-001; CARD-KOR-003-perception_requirement-001; CARD-KOR-003-sidewalk_operation-001; CARD-KOR-004-speed_policy-001; CARD-KOR-004-crosswalk_operation-001 | emergency_stop; perception_requirement; sidewalk_operation; speed_policy; crosswalk_operation | emergencyStopDistanceCm; ttcThresholdSec; safeDistanceCm | ready |
...
| YieldWait | CARD-KOR-003-sidewalk_operation-001; CARD-KOR-004-crosswalk_operation-001 | sidewalk_operation; crosswalk_operation | waitTimeoutSec; lowSpeedZoneSpeedKmh | partial: 대기 시간 초기값은 후속 Policy Config 단계에서 결정 |
