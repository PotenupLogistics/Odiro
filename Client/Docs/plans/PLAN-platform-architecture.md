...
- packaged simulator exe가 아직 없으면 개발 검증 fallback으로 `Client/Tools/RunPreview.ps1 "-Simulate=<SimulationSetupFile>" "-RunId=<RunId>"`를 subprocess로 호출
- `Client/Tools/RunPreview.ps1` fallback은 packaged exe와 같은 public command parameter를 받되 내부적으로 `UnrealEditor.exe <uproject> -game -NoSplash`를 붙여 실행
...
- 패키징 전 환경에서는 `Client/Tools/RunPreview.ps1` fallback으로 같은 `-Simulate`, `-RunId` 인자를 전달해 subprocess 시작
...
- [SimulatorLaunchSubsystem.cpp](x:/UE5/Proto-Unreal/Source/OdiroSim/Private/Platform/SimulatorLaunchSubsystem.cpp): packaged exe 실행, `Client/Tools/RunPreview.ps1` fallback, process start/status failure 분리, status polling
...
- packaged exe가 없을 때 `Client/Tools/RunPreview.ps1` fallback으로 같은 public command parameter 전달
...
T01~T07 완료.
Launcher process는 packaged exe 또는 개발 fallback `Client/Tools/RunPreview.ps1`를 별도 process로 실행한다.
