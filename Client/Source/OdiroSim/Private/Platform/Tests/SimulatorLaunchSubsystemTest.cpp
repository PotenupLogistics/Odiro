...
{
	// Launcher는 simulator public 계약만 넘기고 fixed-step은 experiment_setting에만 둔다.
	const FString simulatorArguments = USimulatorLaunchSubsystem::BuildSimulatorArgumentString(
		TEXT("Json/Experiments/FeatureProbeNoPedestrians"),
		TEXT("run-001"));
	TestTrue(TEXT("simulator passes experiment ref"), simulatorArguments.Contains(TEXT("\"-Experiment=Json/Experiments/FeatureProbeNoPedestrians\"")));
	...
	TestTrue(TEXT("preview passes experiment ref"), previewArguments.Contains(TEXT("\"-Experiment=Json/Experiments/FeatureProbeNoPedestrians\"")));
