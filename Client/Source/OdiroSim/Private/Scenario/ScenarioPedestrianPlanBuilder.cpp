...
bool FScenarioPedestrianPlanBuilder::BuildPlans(
	...
	FScenarioPedestrianPlanBuildResult& outResult)
{
	...
	plannedPedestrians.Sort(
		[](const FScenarioDynamicActorSpec& left, const FScenarioDynamicActorSpec& right)
		{
			return left.InstanceId < right.InstanceId;
		});

	bool bAllSucceeded = true;
	for (const FScenarioDynamicActorSpec& dynamicActorSpec : plannedPedestrians)
	{
		FScenarioPedestrianPlan plan;
...
bool FScenarioPedestrianPlanBuilder::IsPlannedPedestrian(const FScenarioDynamicActorSpec& dynamicActorSpec)
{
	if (dynamicActorSpec.Category != EScenarioActorCategory::Pedestrian) return false;
...
bool FScenarioPedestrianPlanBuilder::TryGetFloatProperty(
...
bool FScenarioPedestrianPlanBuilder::TryGetStringProperty(
	const TMap<FString, FScenarioParamValue>& properties,
	const FString& key,
	FString& outValue)
{
...
bool FScenarioPedestrianPlanBuilder::TryGetVectorProperty(
	const TMap<FString, FScenarioParamValue>& properties,
	const FString& key,
	FVector& outValue)
{
...
FScenarioPedestrianBehaviorParams FScenarioPedestrianPlanBuilder::BuildBehaviorParams(
...
FScenarioPedestrianPathShapeParams FScenarioPedestrianPlanBuilder::BuildPathShapeParams(
	const FScenarioDynamicActorSpec& dynamicActorSpec)
{
...
bool FScenarioPedestrianPlanBuilder::BuildPlanForPedestrian(
	const FScenarioDynamicActorSpec& dynamicActorSpec,
	const FScenarioPedestrianPlanBuildContext& buildContext,
	...
	TArray<FString>& outDiagnostics)
{
	...
	const FScenarioPedestrianPathShapeParams pathShapeParams = BuildPathShapeParams(dynamicActorSpec);
	...
	outPlan = FScenarioPedestrianPlan{};
...
TArray<FVector> FScenarioPedestrianPlanBuilder::BuildPolyline(
	...
	double clearanceCm)
{
	...
	{
		const FScenarioPedestrianObstacleFootprint* Footprint = nullptr;
		double ProjectionCm = 0.0;
	};

	TArray<FBlockingObstacle> blockingObstacles;
	for (const FScenarioPedestrianObstacleFootprint& footprint : obstacleFootprints)
	{
	...
	for (const FBlockingObstacle& blockingObstacle : blockingObstacles)
	{
		...
		const FScenarioPedestrianObstacleFootprint& footprint = *blockingObstacle.Footprint;
...
TArray<FVector> FScenarioPedestrianPlanBuilder::BuildCurvedPolyline(
...
TArray<FVector> FScenarioPedestrianPlanBuilder::BuildTwoPointBezierPolyline(
	const TArray<FVector>& polyline,
	const FString& instanceId,
	const FScenarioPedestrianPathShapeParams& pathShapeParams)
{
...
TArray<FVector> FScenarioPedestrianPlanBuilder::BuildCornerRoundedPolyline(
	const TArray<FVector>& polyline,
	const FScenarioPedestrianPathShapeParams& pathShapeParams)
{
	TArray<FVector> curvedPolyline;
...
bool FScenarioPedestrianPlanBuilder::PathIntersectsFootprints(
	const TArray<FVector>& polyline,
	const TArray<FScenarioPedestrianObstacleFootprint>& obstacleFootprints,
	double clearanceCm)
{
...
void FScenarioPedestrianPlanBuilder::AppendPointIfSeparated(TArray<FVector>& points, const FVector& point)
{
	if (points.IsEmpty()
...
FVector FScenarioPedestrianPlanBuilder::EvaluateCubicBezier(
	const FVector& p0,
	const FVector& p1,
	const FVector& p2,
	const FVector& p3,
...
FVector FScenarioPedestrianPlanBuilder::EvaluateQuadraticBezier(
	const FVector& p0,
	const FVector& p1,
	const FVector& p2,
	double alpha)
...
bool FScenarioPedestrianPlanBuilder::SegmentIntersectsFootprint(
...
void FScenarioPedestrianPlanBuilder::AppendPlanPoints(
	const TArray<FVector>& polyline,
	...
	double& outDurationSeconds)
{
	...
	for (int32 pointIndex = 0; pointIndex < polyline.Num(); ++pointIndex)
	{
		...
		FScenarioPedestrianPlanPoint planPoint;
...
FString FScenarioPedestrianPlanBuilder::BuildResolvedFootprintHash(const TArray<FScenarioPedestrianObstacleFootprint>& obstacleFootprints)
{
	TArray<FScenarioPedestrianObstacleFootprint> sortedFootprints = obstacleFootprints;
	sortedFootprints.Sort(
		[](const FScenarioPedestrianObstacleFootprint& left, const FScenarioPedestrianObstacleFootprint& right)
		{
			return left.InstanceId < right.InstanceId;
		});

	uint32 hash = 0;
	for (const FScenarioPedestrianObstacleFootprint& footprint : sortedFootprints)
	{
...
FString FScenarioPedestrianPlanBuilder::BuildPlanHash(
	const FScenarioDynamicActorSpec& dynamicActorSpec,
	const FScenarioPedestrianPlanBuildContext& buildContext,
	const FString& resolvedFootprintHash,
	const TArray<FScenarioPedestrianPlanPoint>& planPoints)
{
	...
	for (const FScenarioPedestrianPlanPoint& planPoint : planPoints)
	{
...
FString FScenarioPedestrianPlanBuilder::BuildBehaviorHash(const FScenarioPedestrianBehaviorParams& behaviorParams)
{
	uint32 hash = 0;
...
FString FScenarioPedestrianPlanBuilder::BuildPedestrianScenarioHash(const FString& planHash, const FString& behaviorHash)
{
	uint32 hash = 0;
...
FString FScenarioPedestrianPlanBuilder::FormatVectorForHash(const FVector& value)
{
