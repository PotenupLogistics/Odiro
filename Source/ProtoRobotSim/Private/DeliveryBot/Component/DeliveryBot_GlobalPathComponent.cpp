...
bool UDeliveryBot_GlobalPathComponent::BuildPath(const FVector& startLocation, const FVector& goalLocation, const FDeliveryBotNavigationConfigInfo& navigationConfigInfo)
{
	...
	case EDeliveryBotPathFinderType::HybridAStar:
		return BuildPathByHybridAStar(startLocation, goalLocation, navigationConfigInfo.HybridAStarConfigInfo);
...
// Hybrid A*로 차량 헤딩과 전진/후진 방향을 포함한 전역 경로를 생성한다.
bool UDeliveryBot_GlobalPathComponent::BuildPathByHybridAStar(
	...
	const FDeliveryBotHybridAStarConfigInfo& hybridAStarConfigInfo)
{
	...
	const FDeliveryBotHybridAStarConfigInfo normalizedConfigInfo =
		NormalizeHybridAStarConfigInfo(hybridAStarConfigInfo);
	...
	const float startHeadingRadian =
		NormalizeRadian(GetHybridAStarStartHeadingRadian(fixedStartLocation, goalLocation));

	const float goalHeadingRadian =
		NormalizeRadian(GetHybridAStarGoalHeadingRadian(fixedStartLocation, goalLocation));

	const int32 startHeadingIndex =
		GetHybridAStarHeadingIndex(startHeadingRadian, normalizedConfigInfo);

	const FDeliveryBotHybridAStarNodeKey startNodeKey =
		MakeDeliveryBotHybridAStarNodeKey(
			startGridIndex,
			startHeadingIndex,
			EDeliveryBotMoveDirectionType::Forward);

	const FDeliveryBotHybridAStarNodeInfo startNodeInfo =
		MakeDeliveryBotHybridAStarNodeInfo(
			startNodeKey,
			fixedStartLocation,
			startHeadingRadian,
			0.f,
			GetHybridAStarHeuristicCost(fixedStartLocation, goalLocation));

	TMap<FDeliveryBotHybridAStarNodeKey, FDeliveryBotHybridAStarNodeInfo> nodeMap;
	TSet<FDeliveryBotHybridAStarNodeKey> closedSet;
	FDeliveryBotHybridAStarPriorityQueueInfo openQueue;

	nodeMap.Add(startNodeKey, startNodeInfo);
	openQueue.Push(MakeDeliveryBotHybridAStarQueueNodeInfo(startNodeInfo));

	const TArray<EDeliveryBotMoveDirectionType> moveDirectionTypes =
		GetHybridAStarMoveDirectionTypes(normalizedConfigInfo);

	const TArray<float> turnDirectionSigns =
		GetHybridAStarTurnDirectionSigns();

	FDeliveryBotHybridAStarNodeKey goalNodeKey;
	bool bFoundGoal = false;

	int32 searchCount = 0;
	int32 addedNodeCount = 0;

	auto AddNeighborNodes =
		[&](const FDeliveryBotHybridAStarNodeInfo& currentNodeInfo) -> int32
	{
		int32 addedCount = 0;

		for (EDeliveryBotMoveDirectionType moveDirectionType : moveDirectionTypes)
		{
			for (float turnDirectionSign : turnDirectionSigns)
			{
				float nextHeadingRadian = 0.f;

				const FVector nextLocationCm =
					CalculateHybridAStarNextLocation(
						currentNodeInfo.LocationCm,
						currentNodeInfo.HeadingRadian,
						moveDirectionType,
						turnDirectionSign,
						normalizedConfigInfo,
						nextHeadingRadian);

				const FIntPoint nextGridIndex =
					gridSubsystem->GetGridIndexByWorldLocation(nextLocationCm);

				if (!gridSubsystem->IsWalkableGridIndex(nextGridIndex))
					continue;

				if (!CanUseHybridAStarSegment(currentNodeInfo.LocationCm, nextLocationCm, gridSubsystem))
					continue;

				const int32 nextHeadingIndex =
					GetHybridAStarHeadingIndex(nextHeadingRadian, normalizedConfigInfo);

				const float baseMoveCost =
					static_cast<float>(FVector::Dist2D(currentNodeInfo.LocationCm, nextLocationCm));

				float nextContinuousReverseDistanceCm = 0.f;
				int32 nextContinuousReverseStepCount = 0;
				if (moveDirectionType == EDeliveryBotMoveDirectionType::Reverse)
				{
					nextContinuousReverseDistanceCm = currentNodeInfo.Key.MoveDirectionType == EDeliveryBotMoveDirectionType::Reverse
						? currentNodeInfo.ContinuousReverseDistanceCm + baseMoveCost
						: baseMoveCost;

					if (nextContinuousReverseDistanceCm > normalizedConfigInfo.MaxContinuousReverseDistanceCm)
						continue;

					nextContinuousReverseStepCount = currentNodeInfo.Key.MoveDirectionType == EDeliveryBotMoveDirectionType::Reverse
						? currentNodeInfo.Key.ContinuousReverseStepCount + 1
						: 1;
				}

				const FDeliveryBotHybridAStarNodeKey nextNodeKey =
					MakeDeliveryBotHybridAStarNodeKey(
						nextGridIndex,
						nextHeadingIndex,
						moveDirectionType,
						nextContinuousReverseStepCount);

				if (closedSet.Contains(nextNodeKey))
					continue;

				const float nextGCost =
					currentNodeInfo.GCost +
					GetHybridAStarDirectionCost(
						baseMoveCost,
						currentNodeInfo.Key.MoveDirectionType,
						moveDirectionType,
						normalizedConfigInfo,
						turnDirectionSign,
						currentNodeInfo.TurnDirectionSign);

				FDeliveryBotHybridAStarNodeInfo* existingNodeInfo = nodeMap.Find(nextNodeKey);
				if (existingNodeInfo != nullptr && nextGCost >= existingNodeInfo->GCost)
					continue;

				const float nextHCost =
					GetHybridAStarHeuristicCost(nextLocationCm, goalLocation);

				const FDeliveryBotHybridAStarNodeInfo nextNodeInfo =
					MakeDeliveryBotHybridAStarChildNodeInfo(
						nextNodeKey,
						currentNodeInfo.Key,
						nextLocationCm,
						nextHeadingRadian,
						nextGCost,
						nextHCost,
						nextContinuousReverseDistanceCm,
						turnDirectionSign);

				nodeMap.Add(nextNodeKey, nextNodeInfo);
				openQueue.Push(MakeDeliveryBotHybridAStarQueueNodeInfo(nextNodeInfo));
	...
	while (!openQueue.IsEmpty() && searchCount < normalizedConfigInfo.MaxSearchCount)
	{
		FDeliveryBotHybridAStarQueueNodeInfo currentQueueNodeInfo;
		if (!openQueue.Pop(currentQueueNodeInfo))
			break;

		if (closedSet.Contains(currentQueueNodeInfo.Key))
			continue;

		FDeliveryBotHybridAStarNodeInfo* currentNodeInfo =
			nodeMap.Find(currentQueueNodeInfo.Key);
		...
		if (IsHybridAStarGoalReached(
			...
			normalizedConfigInfo))
		{
			goalNodeKey = currentNodeInfo->Key;
			bFoundGoal = true;
			break;
		}

		closedSet.Add(currentNodeInfo->Key);
		addedNodeCount += AddNeighborNodes(*currentNodeInfo);
	}
	...
	if (!bFoundGoal)
	{
		UE_LOG(
			LogTemp,
			Warning,
			TEXT("DeliveryBot Hybrid A* failed. SearchCount: %d, AddedNodeCount: %d"),
			searchCount,
			addedNodeCount);
	...
	TArray<FDeliveryBotHybridAStarNodeInfo> reverseNodePath;
	FDeliveryBotHybridAStarNodeKey traceNodeKey = goalNodeKey;
	...
	for (int32 traceCount = 0; traceCount < maxTraceCount; ++traceCount)
	{
		const FDeliveryBotHybridAStarNodeInfo* traceNodeInfo = nodeMap.Find(traceNodeKey);

		if (traceNodeInfo == nullptr)
			break;

		reverseNodePath.Add(*traceNodeInfo);

		if (!traceNodeInfo->bHasParent)
		{
			bFoundStart = true;
			break;
		}

		traceNodeKey = traceNodeInfo->ParentKey;
	}

	if (!bFoundStart)
		return false;

	TArray<FDeliveryBotHybridAStarNodeInfo> nodePath;
	nodePath.Reserve(reverseNodePath.Num());

	for (int32 pathIndex = reverseNodePath.Num() - 1; pathIndex >= 0; --pathIndex)
	{
		nodePath.Add(reverseNodePath[pathIndex]);
	}

	for (int32 pathIndex = 0; pathIndex < nodePath.Num(); ++pathIndex)
	{
		const FDeliveryBotHybridAStarNodeInfo& nodeInfo = nodePath[pathIndex];

		GlobalPath.Add(nodeInfo.LocationCm);

		FDeliveryBotPathPointInfo pathPointInfo;
		pathPointInfo.LocationCm = nodeInfo.LocationCm;
		pathPointInfo.HeadingRadian = nodeInfo.HeadingRadian;

		if (pathIndex == 0 && nodePath.IsValidIndex(1))
		{
			pathPointInfo.MoveDirectionType = nodePath[1].Key.MoveDirectionType;
		}
		else
		{
			pathPointInfo.MoveDirectionType = nodeInfo.Key.MoveDirectionType;
		}
	...
	UE_LOG(
		LogTemp,
		Warning,
		TEXT("DeliveryBot Hybrid A* success. SearchCount: %d, PathCount: %d"),
		searchCount,
		GlobalPathPointInfos.Num());
