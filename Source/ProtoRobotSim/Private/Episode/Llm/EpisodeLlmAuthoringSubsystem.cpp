#include "Episode/Llm/EpisodeLlmAuthoringSubsystem.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogEpisodeLlmAuthoring, Log, All);
...
void UEpisodeLlmAuthoringSubsystem::Deinitialize()
{
	CancelPendingRequest();
	Super::Deinitialize();
}

bool UEpisodeLlmAuthoringSubsystem::GenerateEpisodeFromPrompt(const FString& prompt, const int32 episodeCount)
{
	if (PendingHttpRequest.IsValid())
	{
		FEpisodeLlmGenerationResult result;
	...
	FString requestBody;
	FEpisodeLlmGenerationResult failureResult;
	...
	TWeakObjectPtr<UEpisodeLlmAuthoringSubsystem> weakThis = this;
	...
	UE_LOG(LogEpisodeLlmAuthoring, Log, TEXT("LLM generation request started | Url: %s"), *request->GetURL());
	return true;
}

void UEpisodeLlmAuthoringSubsystem::CancelPendingRequest()
{
	if (!PendingHttpRequest.IsValid())
	{
		return;
...
void UEpisodeLlmAuthoringSubsystem::HandleGenerateResponse(
...
void UEpisodeLlmAuthoringSubsystem::CompleteRequest(const FEpisodeLlmGenerationResult& result)
{
	LatestResult = result;
	if (result.bSuccess)
	{
		UE_LOG(
			LogEpisodeLlmAuthoring,
			Log,
...
bool UEpisodeLlmAuthoringSubsystem::TryBuildRequestBody(
	const FString& prompt,
	const int32 episodeCount,
	FString& outBody,
	FEpisodeLlmGenerationResult& outFailure) const
{
	outBody.Reset();
	outFailure = FEpisodeLlmGenerationResult{};
...
bool UEpisodeLlmAuthoringSubsystem::TryValidateAndSaveRunQueue(
	const FString& responseBody,
	const int32 responseCode,
	FEpisodeLlmGenerationResult& outResult) const
{
	outResult = FEpisodeLlmGenerationResult{};
...
FString UEpisodeLlmAuthoringSubsystem::BuildUrl(const FString& baseUrl, const FString& endpoint)
{
...
FString UEpisodeLlmAuthoringSubsystem::TruncateForDiagnostic(const FString& value)
{
