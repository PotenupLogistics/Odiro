...
namespace
{
	const TCHAR* MainMenuDefaultWidgetBlueprintClassPath = TEXT("/Game/Widgets/MainMenu/WBP_MainMenu.WBP_MainMenu_C");
}
...
UMainMenuWidget* AMainMenuPlayerController::ShowMainWidget()
{
	...
	if (!widgetClass)
	{
		UE_LOG(LogMainMenuPlayerController, Warning, TEXT("MainWidgetClass could not be resolved."));
		return nullptr;
	}
	...
	if (!MainWidget)
	{
		UE_LOG(LogMainMenuPlayerController, Warning, TEXT("MainWidget create failed | Class: %s"), *GetNameSafe(widgetClass.Get()));
		return nullptr;
	}
	...
	UE_LOG(LogMainMenuPlayerController, Log, TEXT("MainWidget shown | Class: %s"), *GetNameSafe(MainWidget->GetClass()));
	return MainWidget;
}
...
TSubclassOf<UMainMenuWidget> AMainMenuPlayerController::ResolveMainWidgetClass() const
{
	...
	const FSoftClassPath defaultMainWidgetClassPath(MainMenuDefaultWidgetBlueprintClassPath);
	...
	UE_LOG(
		...
		Error,
		TEXT("Default MainMenu widget Blueprint could not be loaded | Path: %s"),
		MainMenuDefaultWidgetBlueprintClassPath);
	return nullptr;
}
