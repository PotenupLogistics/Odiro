...
UMainMenuWidget* AMainMenuPlayerController::ShowMainWidget()
{
	...
	if (!widgetClass)
	{
		UE_LOG(LogMainMenuPlayerController, Warning, TEXT("MainWidgetClass is missing."));
		return nullptr;
	}
	...
	if (!MainWidget)
	{
		UE_LOG(LogMainMenuPlayerController, Warning, TEXT("MainWidget creation failed. Class: %s"), *GetNameSafe(widgetClass.Get()));
		return nullptr;
	}
...
TSubclassOf<UMainMenuWidget> AMainMenuPlayerController::ResolveMainWidgetClass() const
{
	...
	UE_LOG(
		...
		Error,
		TEXT("Default MainMenu widget Blueprint not found. Path: %s"),
