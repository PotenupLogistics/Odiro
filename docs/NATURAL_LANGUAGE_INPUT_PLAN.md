...
Natural-language input can now be accepted through the API shell endpoint:

```text
POST /api/v1/generation/world-config/prompt-package
```

This endpoint returns a prompt package only. It does not call an external LLM and does not generate a World Config payload.
...
- API 기반 입력 예정
- Swagger/Postman 또는 간단한 UI에서 prompt 입력
- CLI는 자연어 입력용이 아니라 JSON 검증용으로만 사용
