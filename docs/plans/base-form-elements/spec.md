...
상태: 디자인 확정 대기 (구현 미착수). 계획은 [plan.md](plan.md) 참조.
...
## 범위
- 추가: Text Input, Slider, Switcher, Dropdown, Toggle Button, Checkbox, Thumbnail Card,
  Tree View, Tooltip, Context Menu.
- 비목표: 신규 색/타이포 토큰 도입(기존 `DA_BaseTokens` 재사용), 화면 단위 기능 결합,
  speculative 옵션/추상화.

## 공통 규칙 (모든 컴포넌트 적용)
- 토큰 출처: `UBaseWidgetTokenCatalog` → `/Game/Widgets/Common/DA_BaseTokens`.
  `BaseWidgetPrivate::ResolveBaseTokenCatalog`로 해석. 하드코딩 색/치수 금지.
...
- 폰트: Freesentation (`/Game/Fonts/Freesentation/Freesentation-{4Regular,5Medium,6SemiBold,7Bold}_Font`),
...
| 항목 | 값 |
|---|---|
| Radius (기본) | 4px (`Radius`), pill `RadiusPill` |
| Border | 1px (`BorderWidth`) |
| Field 높이 | 29px (`FieldHeight`) |
| Control 높이 | 38px (`ControlHeight`), Small 35 (`ControlHeightSmall`) |
| Row 높이 | 30px (`RowHeight`) |
...
### Text Input
- 변형: single line / number / number range / multiline.
...
- Multiline: textbox와 동일 동작(여러 줄, 줄바꿈).

### Slider
- 핸들은 얇은 바(≈4px).
- 변형: value + number field(동기화) · range(dual handle) + paired number-range field
  · disabled(필드 없는 버전).
- number field ↔ 슬라이더 값 양방향 동기화. range는 min/max 두 핸들.
...
### Tooltip
- **흰 배경 + `#1A1A1A` 텍스트**.
- 위치: 툴팁의 **좌하단 모서리 = 커서 tip**(우상단으로 펼침). 화면 끝에서 자동 flip.
- Button/Card 등에 ~1초 hover 시 표시.
